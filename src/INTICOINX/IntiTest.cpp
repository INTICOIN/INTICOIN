...
* @file       Tests.cpp
*
* @brief      Test routines for INTICOINX.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", INTICOINX_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),INTICOINX_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("INTICOINX v%s self-test routine\n", INTICOINX_VERSION_STRING);
	...
	// Run through all of the INTICOINX tests
