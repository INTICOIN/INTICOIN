...
///
/// \brief      Parameter manipulation routines for the INTICOINX cryptographic
///             components.
///
...
/// \brief Fill in a set of INTICOINX parameters from a modulus "N".
...
///
/// \throws         INTICOINXException if the process fails
///
...
/// be known to any party, or the security of INTICOINX is
...
CalculateParams(Params &params, Bignum N, string aux, uint32_t securityLevel)
{
	...
	if (NLen < 1023) {
		throw INTICOINXException("Modulus must be at least 1023 bits");
	}

	// Verify that "securityLevel" is  at least 80 bits (minimum).
	if (securityLevel < 80) {
		throw INTICOINXException("Security level must be at least 80 bits.");
	}
	...
	*pLen = *qLen = 0;
	...
	if (securityLevel < 80) {
		...
		*pLen = 3072;
		...
		throw INTICOINXException("Security level not supported.");
	}

	if (*pLen > maxPLen) {
		throw INTICOINXException("Modulus size is too small for this security level.");
	}
}
...
deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen)
{
	...
	if ((uint32_t)(result.modulus.bitSize()) < pLen ||          // modulus is pLen bits long
		...
		throw INTICOINXException("Group parameters are not valid");
	}
...
deriveIntegerGroupFromOrder(Bignum &groupOrder)
{
	...
	throw INTICOINXException("Too many attempts to generate Schnorr group.");
}
                              ...
                              uint256 *resultPseed, uint256 *resultQseed)
{
	...
	if (qLen > (sizeof(seed)) * 8) {
		...
		// throw INTICOINXException("Seed is too short to support the required security level.");
	}

#ifdef INTICOINX_DEBUG
	...
	*resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
	...
	// Throw an exception.
	throw INTICOINXException("Unable to generate a prime modulus for the group");
}
...
/// \throws                             A INTICOINXException if error.
///
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, Bignum modulus, Bignum groupOrder, uint32_t index)
{
	...
	if (index > 255) {
		throw INTICOINXException("Invalid index for group generation");
	}
	...
	throw INTICOINXException("Unable to find a generator, too many attempts");
}
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 2) {
		throw INTICOINXException("Prime length is too short");
	}
	...
	if (primeBitLen < 33) {
		...
		throw INTICOINXException("Unable to find prime in Shawe-Taylor algorithm");
	...
	throw INTICOINXException("Unable to generate random prime (too many tests)");
}
